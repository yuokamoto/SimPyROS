#!/usr/bin/env python3
"""
Process-Separated URDF Robot Visualizer V2

URDFRobotVisualizerインターフェースに準拠したプロセス分離PyVistaビジュアライザー V2
...
from core.process_separated_pyvista_v2 import ProcessSeparatedPyVistaVisualizerV2, SharedMemoryConfig
from core.simulation_object import Pose


class ProcessSeparatedURDFRobotVisualizerV2(ProcessSeparatedPyVistaVisualizerV2):
    """
    URDFRobotVisualizerインターフェース準拠のプロセス分離ビジュアライザー V2
    ...
    def load_robot(self, robot_name: str, robot_data: Any) -> bool:
        ...
        try:
            ...
            if not self.is_initialized:
                print("🚀 Initializing V2 visualizer...")
                init_success = self.initialize()
                if not init_success:
                    print(f"❌ Failed to initialize V2 visualizer")
                    return False
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'architecture': 'process_separated_v2'
        }
    ...
    def print_performance_summary(self):
        ...
        print("📊 ProcessSeparatedURDFRobotVisualizerV2 Performance:")
...
def create_process_separated_urdf_visualizer_v2(config: Optional[SharedMemoryConfig] = None):
    """
    Factory function to create the URDF visualizer V2
    
    Args:
        config: Shared memory configuration
        
    Returns:
        ProcessSeparatedURDFRobotVisualizerV2 instance
    """
    return ProcessSeparatedURDFRobotVisualizerV2(config)


if __name__ == "__main__":
    # Test the URDF visualizer
    print("🧪 Testing ProcessSeparatedURDFRobotVisualizerV2")
    
    config = SharedMemoryConfig(max_robots=2, max_links_per_robot=8)
    visualizer = ProcessSeparatedURDFRobotVisualizerV2(config)
